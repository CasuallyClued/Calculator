...
#define PASSWORD "1234" // Variable to hold the value of the password (not using flash)
...
/* CHANGES from 1.0:
 * low_level_funcs.c
 * - Update to PrintChar() to allow special characters to be displayed
 * - Improved commenting
 * high_level_funcs.c
 * - New functions PrintDisplayFull() & ClearInputBuffer()
 * - Update to ReadEchoInput() to allow for special characters to be input.
 * - Also utilises new high level functions to increase code efficiency
*/
////////////////
/* CHANGES from 2.0:
 * high_level_funcs.c
 * - New functions CheckPassword()
 * - 		Created so that the calculator requires a correct password to be entered
 * - 		to enable use
 * - 		Ideally the function would use the flash so that the user could specify
 * - 		and configure an input, however as flash manipulation hadn't yet been
 * - 		accomplished, the password is stored and configured in main.c
 * - Update to WelcomeScreen()
 * - 		No longer requires input to advance
 * - 		Better timed animations
 * mid_level_funcs.h
 * - 		Improved commenting
*/
...
/* CHANGES from 3.0:
 * high_level_funcs.c
 * - Improved animations in WelcomeScreen()
 * Improved commenting
*/
